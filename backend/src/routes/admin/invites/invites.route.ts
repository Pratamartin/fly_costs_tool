...
export const index = createRoute({
  ...
  description: 'Returns a list of all generated invite codes with their statuses (`ATIVO`, `USADO`, `EXPIRADO`). `ADMIN` only.',
  tags,
...
export const remove = createRoute({
  ...
  description: 'Invalidates an active invite code by setting `expiresAt` to current time (soft revoke). Cannot revoke codes that are already used (`INVITE_ALREADY_USED`) or expired (`INVITE_ALREADY_EXPIRED`). `ADMIN` only.',
  tags,
